...
def ajouter_des_boites_ou_bonus(arene:dict,val_min:int=-5, val_max:int=2,nb_boites:int=4):
...
def deplacer_joueur(arene:dict,num_joueur:int,direction:str)->str:
    ...
    if direction not in DIRECTIONS:
        mult_div_serpent(arene,serpent.get_liste_pos(joueur),num_joueur,"/")
        serpent.ajouter_points(joueur,MALUS)
        return "Le joueur "+str(num_joueur)+" a joué une direction incorrecte "+direction
...
def copy_arene(arene:dict)->dict:
    ...
    nouvelle_arene = dict()
    nouvelle_arene["matrice"] = arene["matrice"]
    nouvelle_arene["nb_joueurs"] = arene["nb_joueurs"]
    nouvelle_arene["serpents"] = []
    liste_serp = []
    print("\n")
    print(nouvelle_arene)
    print("\n")

    for serp in arene["serpents"]:
        liste_serp.append(serp)
    nouvelle_arene["serpents"] = liste_serp
    return nouvelle_arene


are = Arene(10,10,["Serpent1","Serpent2","Serpent3","Serpent4"])
Serpent1 = serpent.Serpent("Joueur 1",1,0,[(1,1),(1,2),(1,3)],0,0,0,'N')
Serpent2 = serpent.Serpent("Joueur 2",2,0,[(2,1),(2,2),(2,3)],0,0,0,'S')
Serpent3 = serpent.Serpent("Joueur 3",3,0,[(3,1),(3,2),(3,3)],0,0,0,'E')
Serpent4 = serpent.Serpent("Joueur 4",4,0,[(4,1),(4,2),(4,3)],0,0,0,'W')
#copie = copy_arene(are)
#print(are)